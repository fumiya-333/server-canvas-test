...
export const Index: FC<Props> = () => {
  ...
  const execJoinCanvas = useCallback(async (value: string) => {
    ...
    const refs  = [canvas2Ref, canvas2Ref]
    for(const ref of refs) {
      const image = await ref.current?.getImagefromCanvas()
      canvasJoinRef.current?.drawImage(image, 0, 0, canvasJoinRef.current.getWidth(), canvasJoinRef.current.getHeight())
    }
  ...
  useEffect(()=> {
    (async() => {
      ...
      canvasRef.current?.init(500, 300, 'c-canvas u-none')
      canvas2Ref.current?.init(500, 300, 'c-canvas2 u-none')
      await execJoinCanvas(text1)
    })()
